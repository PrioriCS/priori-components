...
function LinkButton(_ref) {
  var {
      ...
      replace,
      only,
      component: Component = _.Button,
      className = '',
      disabled
    } = _ref,
    rest = _objectWithoutProperties(_ref, _excluded);
  var visit = _react.default.useCallback(event => {
    onClick(event);
    if ((0, _inertia.shouldIntercept)(event)) {
      event.preventDefault();
      _inertia.Inertia.visit(href, {
        preserveScroll,
        preserveState,
        replace,
        only
  ...
  return /*#__PURE__*/(0, _jsxRuntime.jsx)(Component, _objectSpread({
    onClick: visit,
    className: "".concat(disabled ? '' : 'cursor-pointer', " ").concat(className, " "),
    disabled: disabled
  }, rest));
}